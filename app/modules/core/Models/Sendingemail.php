...
class Core_Model_Sendingemail
{
    ...
    public function enviarcorreo($data)
    {
        ...
        //$this->email->getMail()->addAddress($correo,  "Contacto Feroldán");
        $this->email->getMail()->addBCC("desarrollo8@omegawebsystems.com",  "Contacto Feroldán");
        $this->email->getMail()->addBCC("juansesdvsf@gmail.com",  "Contacto Feroldán");
        ...
        $this->email->getMail()->Subject = "Contáctenos Feroldán";
        $this->email->getMail()->msgHTML($content);
        $this->email->getMail()->AltBody = $content;
        if ($this->email->sed() == true) {
            return 1;
    ...
    public function confirmacion($data)
    {
        ...
        $informacion = $informacionModel->getList("", "orden ASC")[0];
        $correo = $informacion->info_pagina_correos_contacto;
        $this->email->getMail()->addAddress($correo,  "Solicitud De Clasificado");
        $this->email->getMail()->addBCC("soporteomega@omegawebsystems.com",  "Soporte omega");
        $this->email->getMail()->setFrom($data['email'], $data['nombre']);
