...
class Administracion_fotosController extends Administracion_mainController
{
	...
	public function insertAction(){
		...
		if (Session::getInstance()->get('csrf')[$this->_getSanitizedParam("csrf_section")] == $csrf ) {	
			...
			if($_FILES['fotos_imagen']['name'] != ''){
				$data['fotos_imagen'] = $uploadImage->upload("fotos_imagen");
			}
	...
	public function updateAction(){
		...
		if (Session::getInstance()->get('csrf')[$this->_getSanitizedParam("csrf_section")] == $csrf ) {
			...
			if ($content->fotos_id) {
				...
				$uploadImage =  new Core_Model_Upload_Image();
				if($_FILES['fotos_imagen']['name'] != ''){
					if($content->fotos_imagen){
						$uploadImage->delete($content->fotos_imagen);
					}
					$data['fotos_imagen'] = $uploadImage->upload("fotos_imagen");
				} else {
					$data['fotos_imagen'] = $content->fotos_imagen;
				}
	...
	public function deleteAction()
	{
		...
		if (Session::getInstance()->get('csrf')[$this->_csrf_section] == $csrf ) {
			...
			if (isset($id) && $id > 0) {
				...
				if (isset($content)) {
					$uploadImage =  new Core_Model_Upload_Image();
					if (isset($content->fotos_imagen) && $content->fotos_imagen != '') {
						$uploadImage->delete($content->fotos_imagen);
					}
	...
	{
		$data = array();
		$data['fotos_nombre'] = $this->_getSanitizedParam("fotos_nombre");
		$data['fotos_imagen'] = "";
		if($this->_getSanitizedParam("fotos_album") == '' ) {
			$data['fotos_album'] = '0';
		} else {
    ...
    protected function getFilter()
    {
        ...
        if (Session::getInstance()->get($this->namefilter)!="") {
            $filters =(object)Session::getInstance()->get($this->namefilter);
            if ($filters->fotos_nombre != '') {
                $filtros = $filtros." AND fotos_nombre LIKE '%".$filters->fotos_nombre."%'";
            }
            if ($filters->fotos_imagen != '') {
                $filtros = $filtros." AND fotos_imagen LIKE '%".$filters->fotos_imagen."%'";
            }
    ...
    protected function filters()
    {
        if ($this->getRequest()->isPost()== true) {
            ...
            $parramsfilter = array();
					$parramsfilter['fotos_nombre'] =  $this->_getSanitizedParam("fotos_nombre");
					$parramsfilter['fotos_imagen'] =  $this->_getSanitizedParam("fotos_imagen");
